...
public interface ShowRepository extends JpaRepository<Show, Integer> {
    ...
    @Query(value = "SELECT * FROM shows WHERE movie_id = ?1 AND theatre_id = ?2 AND date >= CURRENT_DATE", nativeQuery = true)
    public List<Show> getAllShowsOfMovieInTheater(Integer movieId, Integer theaterId);

    @Query(value = "SELECT * FROM shows WHERE movie_id = ?1 AND date >= CURRENT_DATE", nativeQuery = true)
    public List<Show> getAllShowsOfMovie(Integer movieId);

    @Query(value = "SELECT * FROM shows WHERE date >= CURRENT_DATE", nativeQuery = true)
